...
トップページの「最新記事」には、サイト全体で最も新しい `date` のニュースだけを表示します。古い日付の記事は、カテゴリページ、月別ページ、日別詳細ページから閲覧します。

この挙動は `src/config/site.ts` の `homepage.showLatestDateOnly` で管理しています。
